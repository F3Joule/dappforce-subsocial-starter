...
| `--substrate-cors "URL list"`      | Override default RPC-cors for Substrate node (e.g. `--substrate-cors "http://localhost,https://polkadot.js.org"`). By default CORS include `https://app.subsocial.network`, `https://polkadot.js.org`, `https://polkaverse.com` and `https://sub.id`. |
